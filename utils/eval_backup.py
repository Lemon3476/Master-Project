...
def _get_global_positions(motion, skeleton):
    """一个独立的辅助函数，用于从motion数据中计算全局关节位置。"""
    B, T, D = motion.shape
...
def _get_normalized_global_positions(motion, skeleton, l2p_mean, l2p_std, ctx_frames=10):
    """获取归一化的全局关节位置"""
    gp = _get_global_positions(motion, skeleton)
    gp = gp[:, ctx_frames:-1]
    return (gp - l2p_mean) / l2p_std

def _get_q_rotations(motion, ctx_frames=10):
    """一个独立的辅助函数，用于从motion数据中获取处理过的四元数旋转。"""
    B, T, D = motion.shape
    rot, _ = torch.split(motion, [D-3, 3], dim=-1)
    rot = trf.t_quat.from_ortho6d(rot.reshape(B, T, -1, 6))
    rot = ops.remove_quat_discontinuities(rot)
    rot = rot[:, ctx_frames:-1]
    return rot

def _get_npss_features(motion, ctx_frames=10):
    """一个独立的辅助函数，用于计算NPSS所需的特征。"""
    B, T, D = motion.shape
    rot, _ = torch.split(motion, [D-3, 3], dim=-1)
    rot = trf.t_quat.from_ortho6d(rot.reshape(B, T, -1, 6))
...
def _get_foot_velocity(motion, skeleton, foot_ids):
    """计算脚部速度"""
    B, T, D = motion.shape
...
# benchmark metrics
#####################################
def l2p(GT_motion, pred_motion, skeleton, l2p_mean, l2p_std, ctx_frames=10, return_std=False):
    """
    计算L2P度量：全局关节位置的L2距离。
    ...
    Args:
        ...
        pred_motion: 预测动作数据
        skeleton: 骨骼数据
        l2p_mean, l2p_std: 用于归一化的统计数据
        ctx_frames: 上下文帧数
        return_std: 是否同时返回标准差
        
    Returns:
        mean_val: 平均L2P度量
        std_val: (可选) 标准差
    """
    # 调用独立的辅助函数获取归一化的全局关节位置
    GT_gp = _get_normalized_global_positions(GT_motion, skeleton, l2p_mean, l2p_std, ctx_frames)
    pred_gp = _get_normalized_global_positions(pred_motion, skeleton, l2p_mean, l2p_std, ctx_frames)

    # 计算每个序列的平均误差
    norm_per_seq = torch.mean(torch.sqrt(torch.sum((GT_gp - pred_gp) ** 2, dim=(2, 3))), dim=1)
    
    mean_val = torch.mean(norm_per_seq).item()
    if not return_std:
        return mean_val
    
    std_val = torch.std(norm_per_seq).item()
    return mean_val, std_val

def l2q(GT_motion, pred_motion, ctx_frames=10, return_std=False):
    """
    计算L2Q度量：关节旋转的L2距离。
    ...
    """
    # 调用独立的辅助函数获取四元数旋转
    GT_rot = _get_q_rotations(GT_motion, ctx_frames)
    pred_rot = _get_q_rotations(pred_motion, ctx_frames)

    # 计算每个序列的平均误差
    norm_per_seq = torch.mean(torch.sqrt(torch.sum((GT_rot - pred_rot) ** 2, dim=(2, 3))), dim=1)
...
def npss(GT_motion, pred_motion, ctx_frames=10, return_std=False):
    """
    计算NPSS度量：动作频率特性的差异度量。
    ...
    """
    # 调用独立的辅助函数获取NPSS特征
    GT_cdf_power, GT_total_power = _get_npss_features(GT_motion, ctx_frames)
    pred_cdf_power, _ = _get_npss_features(pred_motion, ctx_frames)

    # 计算每个序列的误差
    emd = torch.norm((pred_cdf_power - GT_cdf_power), p=1, dim=1)
    power_weighted_emd = torch.sum(emd * GT_total_power, dim=1) / (torch.sum(GT_total_power, dim=1) + 1e-8)
    
    mean_val = torch.mean(power_weighted_emd).item()
    if not return_std:
        return mean_val
        
    std_val = torch.std(power_weighted_emd).item()
    return mean_val, std_val

def foot_skate(pred_motion, pred_contact, skeleton, foot_ids, ctx_frames=10, return_std=False):
    """
    计算脚部滑动度量：脚部与地面接触时的移动量。
    ...
    """
    # 调用独立的辅助函数获取脚部速度
    fv = _get_foot_velocity(pred_motion, skeleton, foot_ids)
    
    # 计算每个序列的误差
    metric_per_seq = torch.mean(torch.sum(fv * pred_contact, dim=-1)[:, ctx_frames:-1], dim=-1)
    
    mean_val = torch.mean(metric_per_seq).item() * 100
    if not return_std:
        return mean_val
        
    std_val = torch.std(metric_per_seq).item() * 100
    return mean_val, std_val
...
def traj_pos_error(GT_traj, pred_motion, ctx_frames=10, return_std=False):
    """
    计算轨迹位置误差：预测轨迹与目标轨迹的差异。
    ...
    norm_per_seq = torch.mean(norm[:, ctx_frames:-1], dim=-1)
    
    mean_val = torch.mean(norm_per_seq).item() * 100
    if not return_std:
        return mean_val
        
    std_val = torch.std(norm_per_seq).item() * 100
    return mean_val, std_val