...
def get_output_directory(args, mode_override=None):
    ...
    # Remove this - we'll add the with_keyframes suffix only to subdirectories
    # if args.show_keyframes:
    #     mode_parts.append("with_keyframes")
...
if __name__ =="__main__":
    ...
    # Load all batches in order, preserving GT order
    print("Loading all batches to ensure consistent ordering...")
    ...
    if use_segment_mode:
        ...
        # Get results from baseline methods first
        baseline_batches = []
        for res in evaluator.eval(traj_option=args.traj_edit):
            baseline_batches.append(res)
            
            # Stop after loading the required batch
            if len(baseline_batches) > args.batch_id:
                break
        ...
        for res in evaluator.eval(traj_option=args.traj_edit):
            ...
            # Stop after loading the required batch
            if len(all_results) > args.batch_id:
                break
