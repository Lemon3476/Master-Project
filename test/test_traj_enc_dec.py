...
def test_full_pipeline(kf_config_path, ref_config_path):
    ...
    # 6. 准备 RefineNet 的输入
    # 在实际流程中，s_R 是对 kf_res['motion'] 中的关键帧进行插值的结果。
    # 在本测试中，为简化流程，我们直接使用 kf_res['motion'] 作为 s_R。
    s_R = kf_res['motion'] 
    p_K = kf_res['phase'] if ref_config.use_phase else None
    # RefineNet 的输入不包含 score，所以我们不传递它
    
    # 7. 执行 RefineNet 的前向传播
    try:
        with torch.no_grad():
            # 注意：RefineNet的forward签名不需要train=False
