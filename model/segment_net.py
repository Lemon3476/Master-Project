...
class SegmentNet(nn.Module):
    def __init__(self, config, dataset: MotionDataset):
        ...
        self.overlap_frames = config.overlap_frames
    ...
    def get_data_mask(self, num_frames, midway_targets):
        """
        Generates a mask indicating which frames are known (1) vs unknown (0).
        For SegmentNet, the first 'overlap_frames' and the last frame are known,
        along with any specified midway targets.
        """
        # 0 for unknown and 1 for known
        data_mask = torch.zeros((num_frames, self.config.d_mask), dtype=torch.float32)
        
        # Mark the first overlap_frames as known
        data_mask[:self.overlap_frames, :] = 1
        
        # Mark the last frame as known
        data_mask[-1, :] = 1
        
        # Mark any additional specified midway targets as known
    ...
    def get_keyframe_pos_indices(self, num_frames):
        # position index relative to overlap and target frame
        ctx_idx = torch.arange(num_frames, dtype=torch.float32)
        ctx_idx = ctx_idx - (self.overlap_frames - 1)
    ...
    def forward(self, motion, midway_targets, phase=None, traj=None, attention_mask=None):
        ...
        x, contact = torch.split(x, [M, self.d_contact], dim=-1)
